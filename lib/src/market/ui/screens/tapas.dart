...
class TapasScreen extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => _TapasScreenState();
...
class _TapasScreenState extends State<TapasScreen>{
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Cart>(builder: (context, cart,child){
      ...
      body: GridView.builder(
        itemCount: ListProducts().Tapas.length,
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: _ListWidgetsShop.imageHeader(
                        image: imagesRoutes().imageTapa+ListProducts().Tapas[index]['Image'].toString(),
                        infoaction:(){
                          _ListWidgetsShop.dialogInfoadd(
                            context: context,
                            image:imagesRoutes().imageTapa+ListProducts().Tapas[index]['Image'].toString(),
                            title: ListProducts().Tapas[index]['Title'].toString(),
                            description2: ListProducts().Tapas[index]['Description2'].toString(),
                            button: _ListWidgetsShop.addCart(action: (){
                              cart.add(image: 'tapas/'+ListProducts().Tapas[index]['Image'].toString(), 
                              title: ListProducts().Tapas[index]['Title'].toString(), 
                              description: ListProducts().Tapas[index]['Description1'].toString(),
                              description2: ListProducts().Tapas[index]['Description2'].toString()
                              );
                              }),
                            );
