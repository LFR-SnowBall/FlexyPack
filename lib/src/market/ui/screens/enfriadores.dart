...
class EnfriadoresScreen extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => _EnfriadoresScreenState();
    // TODO: implement createState
}

class _EnfriadoresScreenState extends State<EnfriadoresScreen>{
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Cart>(builder: (context, cart,child){
      ...
      appBar: AppBar(
        ...
        title: Text('Enfriadores'),
        centerTitle: true,
      ),
      body: GridView.builder(
        itemCount: ListProducts().Enfriadores.length,
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: _ListWidgetsShop.imageHeader(
                        image: imagesRoutes().imageEnfriadores+ListProducts().Enfriadores[index]['Image'].toString(),
                        infoaction:(){
                          _ListWidgetsShop.dialogInfoadd(
                            context: context,
                            image:imagesRoutes().imageEnfriadores+ListProducts().Enfriadores[index]['Image'].toString(),
                            title: ListProducts().Enfriadores[index]['Title'].toString(),
                            description2: ListProducts().Enfriadores[index]['Description2'].toString(),
                            button: _ListWidgetsShop.addCart(action: (){
                              cart.add(image: 'enfriadores/'+ListProducts().Enfriadores[index]['Image'].toString(), 
                              title: ListProducts().Enfriadores[index]['Title'].toString(), 
                              description: ListProducts().Enfriadores[index]['Description1'].toString(),
                              description2: ListProducts().Enfriadores[index]['Description2'].toString()
                              );
                              }),
                            );
