...
class SellosScreen extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => _SellosScreenState();
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Cart>(builder: (context, cart,child){
      ...
      body: GridView.builder(
        itemCount: ListProducts().Sellos.length,
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: _ListWidgetsShop.imageHeader(
                        image: imagesRoutes().imageSello+ListProducts().Sellos[index]['Image'].toString(),
                        infoaction:(){
                          _ListWidgetsShop.dialogInfoadd(
                            context: context,
                            image:imagesRoutes().imageSello+ListProducts().Sellos[index]['Image'].toString(),
                            title: ListProducts().Sellos[index]['Title'].toString(),
                            description2: ListProducts().Sellos[index]['Description2'].toString(),
                            button: _ListWidgetsShop.addCart(action: (){
                              cart.add(image: 'sellos/'+ListProducts().Sellos[index]['Image'].toString(), 
                              title: ListProducts().Sellos[index]['Title'].toString(), 
                              description: ListProducts().Sellos[index]['Description1'].toString(),
                              description2: ListProducts().Sellos[index]['Description2'].toString()
                              );
                              }),
                            );
