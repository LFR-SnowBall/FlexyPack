...
class DivisiondeMetalesScreen extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => _DivisiondeMetalesScreenState();
...
class _DivisiondeMetalesScreenState extends State<DivisiondeMetalesScreen>{
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Cart>(builder: (context, cart,child){
      ...
      appBar: AppBar(
        ...
        title: Text('División de Metales'),
        centerTitle: true,
      ),
      body: GridView.builder(
        itemCount: ListProducts().DivisiondeMetales.length,
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: _ListWidgetsShop.imageHeader(
                        image: imagesRoutes().imageDivisiondeMetal+ListProducts().DivisiondeMetales[index]['Image'].toString(),
                        infoaction:(){
                          _ListWidgetsShop.dialogInfoadd(
                            context: context,
                            image:imagesRoutes().imageDivisiondeMetal+ListProducts().DivisiondeMetales[index]['Image'].toString(),
                            title: ListProducts().DivisiondeMetales[index]['Title'].toString(),
                            description2: ListProducts().DivisiondeMetales[index]['Description2'].toString(),
                            button: _ListWidgetsShop.addCart(action: (){
                              cart.add(image: 'divisiondemetales/'+ListProducts().DivisiondeMetales[index]['Image'].toString(), 
                              title: ListProducts().DivisiondeMetales[index]['Title'].toString(), 
                              description: ListProducts().DivisiondeMetales[index]['Description1'].toString(),
                              description2: ListProducts().DivisiondeMetales[index]['Description2'].toString()
                              );
                              }),
                            );
