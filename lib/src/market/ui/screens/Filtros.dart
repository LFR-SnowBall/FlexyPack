...
class FiltrosScreen extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => _FiltrosScreenState();
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Cart>(builder: (context, cart,child){
      ...
      body: GridView.builder(
        itemCount: ListProducts().Filtros.length,
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: _ListWidgetsShop.imageHeader(
                        image: imagesRoutes().imageFiltros+ListProducts().Filtros[index]['Image'].toString(),
                        infoaction:(){
                          _ListWidgetsShop.dialogInfoadd(
                            context: context,
                            image:imagesRoutes().imageFiltros+ListProducts().Filtros[index]['Image'].toString(),
                            title: ListProducts().Filtros[index]['Title'].toString(),
                            description2: ListProducts().Filtros[index]['Description2'].toString(),
                            button: _ListWidgetsShop.addCart(action: (){
                              cart.add(image: 'filtros/'+ListProducts().Filtros[index]['Image'].toString(), 
                              title: ListProducts().Filtros[index]['Title'].toString(), 
                              description: ListProducts().Filtros[index]['Description1'].toString(),
                              description2: ListProducts().Filtros[index]['Description2'].toString()
                              );
                              }),
                            );
