...
class InsumosVariosScreen extends StatefulWidget{
  @override
  State<StatefulWidget> createState() => _InsumosVariosScreenState();
...
class _InsumosVariosScreenState extends State<InsumosVariosScreen>{
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Cart>(builder: (context, cart,child){
      ...
      appBar: AppBar(
        ...
        title: Text('Insumos Varios'),
        centerTitle: true,
      ),
      body: GridView.builder(
        itemCount: ListProducts().InsumosVarios.length,
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: _ListWidgetsShop.imageHeader(
                        image: imagesRoutes().imageInsumosVarios+ListProducts().InsumosVarios[index]['Image'].toString(),
                        infoaction:(){
                          _ListWidgetsShop.dialogInfoadd(
                            context: context,
                            image:imagesRoutes().imageInsumosVarios+ListProducts().InsumosVarios[index]['Image'].toString(),
                            title: ListProducts().InsumosVarios[index]['Title'].toString(),
                            description2: ListProducts().InsumosVarios[index]['Description2'].toString(),
                            button: _ListWidgetsShop.addCart(action: (){
                              cart.add(image: 'insumosvarios/'+ListProducts().InsumosVarios[index]['Image'].toString(), 
                              title: ListProducts().InsumosVarios[index]['Title'].toString(), 
                              description: ListProducts().InsumosVarios[index]['Description1'].toString(),
                              description2: ListProducts().InsumosVarios[index]['Description2'].toString()
                              );
                              }),
                            );
